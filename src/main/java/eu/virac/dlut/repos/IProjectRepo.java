...
public interface IProjectRepo extends CrudRepository<Project, Integer> {
	...
	@Query(value = "SELECT DISTINCT p.id_fin_source, p.code, p.title, p.accounting_acronym, p.contract_no, p.end, p.proj_lenght_months, p.start, p.id_proj_cat, p.id_proj_char, p.is_active FROM project p "
			+ " JOIN finance_order_work fow ON id_fin_source=fow.id_project "
			...
			+ " AND emp.id_employee = ?3 "
			+ "GROUP BY p.id_fin_source", nativeQuery = true)
